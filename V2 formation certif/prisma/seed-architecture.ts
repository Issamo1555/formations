/**
 * Seed: Add Examples, Exercises, and Quizzes to Architecture course.
 * Usage: npx tsx prisma/seed-architecture.ts
 */
...
async function main() {
  console.log('🚀 Seeding Architecture course (examples + exercises + quizzes)...\n');
  const course = await prisma.course.findUnique({ where: { slug: 'architecture' } });
  if (!course) { console.error('Course "architecture" not found!'); return; }
  ...
  for (const [orderStr, d] of Object.entries(data)) {
    ...
    if (!lesson) { console.log(`⚠ Lesson #${order} not found`); continue; }
    ...
    console.log(`✅ #${order} "${lesson.titleFr}" → exemple + exercice + quiz`);
    updated++;
  }
